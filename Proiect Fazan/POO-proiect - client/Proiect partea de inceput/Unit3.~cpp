...
Form1->ClientSocket->Socket->SendText(text);
brnNameSelect->Hide();
}
//---------------------------------------------------------------------------
...
for(int i=0;i<jucator1->getContor()+1;i++){
...
void __fastcall TForm2::btnLiteraInceputClick(TObject *Sender)
{
btnLiteraInceput->Visible=false;
srand(time(NULL));
char deTrimis = 'a' + rand()%26;
Form1->ClientSocket->Socket->SendText(deTrimis);
}
//---------------------------------------------------------------------------
...
switch(btnNuStiu->Tag){
 case 0:jucator1->setFazan("f");
        ...
        Form1->ClientSocket->Socket->SendText("Litere:"+jucator1->getFazan());
        btnNuStiu->Visible = false;
        sendCuvantButton->Visible = false;
        break;
...
if(jucator2->getFazan().Length() == 5){
 ...
 if(jucator1->getScor() == 2){
   ...
   labelLitereInamic->Visible = false;
